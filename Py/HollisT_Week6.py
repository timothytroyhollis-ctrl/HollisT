...
def get_temperature_unit():
    ...
    while True:
        unit = input(
            'Enter temperature unit (C for Celsius, F for Fahrenheit): ').strip().upper()
        if unit in ['C', 'F']:
            return unit
        elif unit == '':
...
def analyze_temps(temps, unit):
    ...
    converted_range = f'{converted_low:.1f}°{alt_unit} - {converted_high:.1f}°{alt_unit}'
    sorted_temps = sorted(temps)
    median = sorted_temps[len(sorted_temps) // 2] if len(sorted_temps) % 2 == 1 else \
        (sorted_temps[len(sorted_temps) // 2 - 1] +
         sorted_temps[len(sorted_temps) // 2]) / 2
...
def main():
    ...
    print('=' * 78)
    print('Please input each temperature individually, and press ENTER to input the next.')
    print('When complete, press ENTER with no value to finish input and begin analysis.')
    ...
    try:
        ...
        while True:
            try:
                ...
                temps.append(temp)
                print(
                    f'✓ Temperature {temp:.1f}°{unit} added. ({len(temps)} total)')
        ...
        if not temps:
            print('\nNo temperatures were entered. Exiting program.')
            return
